...
### URL（公開済み）
- Privacy Policy URL: **`https://cryptvault.1qaz.jp/privacy`**（本番VPS、nginx + Let's Encrypt、稼働中）
- Support URL: `https://cryptvault.1qaz.jp/`（同上。トップも同じプライバシーページを表示）
- Marketing URL（任意）: 同上
...
| mac_01_folders.png | Mac（ネイティブ, ウインドウ1280×800pt） | 2560×1600 | フォルダ一覧 |
| mac_02_grid.png | 〃 | 2560×1600 | サムネイルグリッド |
| mac_03_settings.png | 〃 | 2560×1600 | 設定（ローカル/WebDAV） |

- **iPhone 6.9"**・**iPad 13"**・**Mac 2560×1600** すべて App Store の必須サイズを満たします。
- Mac 版は AppleScript で `System Events` からウインドウの position/size を厳密に `{1280, 800}` に固定し、
  `screencapture -l<windowID>`（ウインドウID指定）でデスクトップ・他アプリを写さずアプリのウインドウだけを撮影。
- `screenshots/` は `.gitignore` 対象（App Store Connect へのアップロード用ステージングで、リポジトリには含めない）。
...
- [x] アプリアイコン（`Sources/Assets.xcassets/AppIcon` に同梱済み）
- [x] プライバシーポリシーURLを公開（`https://cryptvault.1qaz.jp/privacy`、稼働中）
- [ ] App Privacy「データを収集しない / Data Not Collected」を選択
- [ ] 輸出コンプライアンス（§7 の回答）
- [x] スクショ（iPhone 6.9" / iPad 13" / Mac 2560×1600）`screenshots/` に取得済み
